...
<div class="max-w-2xl space-y-6">
        ...
        <form action="{{ route('buyer.profile.update') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" id="firstName" name="firstName" value="{{ old('firstName', $buyer->firstName) }}"
                        class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest focus:border-transparent outline-none" required>
                </div>
                <div>
                    <label for="midName" class="block text-sm font-medium text-gray-700 mb-2">Middle Name</label>
                    <input type="text" id="midName" name="midName" value="{{ old('midName', $buyer->midName) }}"
                ...
                <div>
                    ...
                    <input type="text" id="lastName" name="lastName" value="{{ old('lastName', $buyer->lastName) }}"
                        class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest focus:border-transparent outline-none" required>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <form action="{{ route('buyer.password.update') }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
